...
file_path = r'c:\Users\Ndree\Downloads\Dicoding Project\fe-mindease-main\fe-mindease-main\src\pages\AdminDashboard.jsx'
...
state_vars = """  const [demotingUserId, setDemotingUserId] = useState(null);
...
"""
content = content.replace("  const [demotingUserId, setDemotingUserId] = useState(null);", state_vars)

# 3. Update fetchData
fetch_logic_old = """      const [statsRes, postsRes, usersRes, analyticsRes] = await Promise.all([
...
fetch_logic_new = """      const [statsRes, postsRes, usersRes, analyticsRes, settingsRes, doctorsRes] = await Promise.all([
        fetch(`${API_URL}/admin/stats`, { headers }),
        fetch(`${API_URL}/admin/posts`, { headers }),
        fetch(`${API_URL}/admin/users`, { headers }),
        fetch(`${API_URL}/admin/analytics`, { headers }),
...
parse_logic_old = """      if (analyticsRes.ok) setAnalytics(await analyticsRes.json());
      else failures.push('analitik');"""

parse_logic_new = """      if (analyticsRes.ok) setAnalytics(await analyticsRes.json());
      else failures.push('analitik');
      if (settingsRes.ok) {
...
content = content.replace(parse_logic_old, parse_logic_new)
...
content = content.replace("      {/* Stats Cards */}", tabs_ui + "\n      {/* Stats Cards */}")
...
content = content[:idx] + extra_panels + content[idx:]
